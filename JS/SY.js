...
document.addEventListener("DOMContentLoaded", async function () {
  ...
  // Fetch data from Firestore
   ...
   const syCollectionRef = collection(db, `students/${userEmail}/SY`);
   const sySnapshot = await getDocs(syCollectionRef);

  if (!sySnapshot.empty) {
     // Assuming there's only one school entry, or select the first document
   const firstDoc = sySnapshot.docs[0];
   const schoolName = firstDoc.id; // Document ID is the school name
   const syData = firstDoc.data();
    ...
    if (syData.Subjects && syData.Semester1 && syData.Semester2) {
      syData.Subjects.forEach((subject, index) => {
        ...
        row.innerHTML = `
          ...
          <td><input type="text" id="s1g" value="${syData.Semester1[index]}" class="w-full px-4 py-2 rounded-lg bg-gray-800 focus:outline-none focus:bg-gray-700 transition duration-200"></td>
          <td><input type="text" id="s2g" value="${syData.Semester2[index]}" class="w-full px-4 py-2 rounded-lg bg-gray-800 focus:outline-none focus:bg-gray-700 transition duration-200"></td>
        `;
...
document.getElementById("submit").addEventListener("click", async function (e) {
  ...
  const syDocRef = doc(db, `students/${userEmail}/SY/${school}`);

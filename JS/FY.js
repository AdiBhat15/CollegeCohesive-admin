...
document.addEventListener("DOMContentLoaded", async function () {
  ...
  // Fetch data from Firestore
   ...
   // Reference to the FY subcollection
   const fyCollectionRef = collection(db, `students/${userEmail}/FY`);
   const fySnapshot = await getDocs(fyCollectionRef);

  if (!fySnapshot.empty) {
     // Assuming there's only one school entry, or select the first document
   const firstDoc = fySnapshot.docs[0];
   const schoolName = firstDoc.id; // Document ID is the school name
   const fyData = firstDoc.data();


    // Pre-fill form fields
    schoolField.value = schoolName || "";
    curriculumField.value = fyData.Curriculum || "";

    // Populate subject rows
    if (fyData.Subjects && fyData.Semester1 && fyData.Semester2) {
      fyData.Subjects.forEach((subject, index) => {
        ...
        row.innerHTML = `
          ...
          <td><input type="text" id="s1g" value="${fyData.Semester1[index]}" class="w-full px-4 py-2 rounded-lg bg-gray-800 focus:outline-none focus:bg-gray-700 transition duration-200"></td>
          <td><input type="text" id="s2g" value="${fyData.Semester2[index]}" class="w-full px-4 py-2 rounded-lg bg-gray-800 focus:outline-none focus:bg-gray-700 transition duration-200"></td>
        `;
